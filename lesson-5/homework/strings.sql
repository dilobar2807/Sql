...
--4-TASK.  Write a query to find the intersection of Products and Products_Discounted 
--tables using INTERSECT.
Select * from Products
INTERSECT
Select * from Products_Discounted

--5-TASK. Write a query to select distinct customer names and their corresponding 
--Country using SELECT DISTINCT.

select distinct FirstName, Country from Customers

--6. Write a query that uses CASE to create a conditional column that displays 'High' if
--Price > 1000, and 'Low' if Price <= 1000 from Products table.
...
--9. Write a query that returns the difference between the Products and Products_Discounted
--tables using EXCEPT.
...
--11. Write a query to find employees in the Employees table who have either Age < 25 
--or Salary > 60000.

select*from  Employees
where Age<25 and Salary>60000

--12. Update the salary of an employee based on their department, increase by
--10% if they work in 'HR' or EmployeeID = 5
...
--14. Use EXCEPT to find customers' ID who have placed orders but do not have a
--corresponding record in the Sales table.
...
--15. Write a query that uses a CASE statement to determine the discount percentage
--based on the quantity purchased. Use orders table. Result set should show customerid,
--quantity and discount percentage. The discount should be applied as follows: 1 item: 3% 
--Between 1 and 3 items : 5% Otherwise: 7%
