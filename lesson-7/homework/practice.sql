...
select category, avg(price)  as avg_prc from Products
group by category
...
--8)Write a query to calculate the total sales per Customer in the Sales table.
select customerID, sum(saleAmount) as sale_amt from Sales
group by CustomerID
--9)Write a query to show the minimum and maximum Price of products grouped by Category. 
--Use products table.
select category, min(price) as min_prc, max(price) as max_prc from Products 
group by category

--11) Write a query to calculate the total sales and average sales for each product category from 
--the Sales table.
select SaleID, sum(SaleAmount) as sum_sale, avg(SaleAmount) as avg_amt from Sales
group by SaleID

--17) Write a query that calculates the total sales for each year in the Sales table.
select saleDate, sum(SaleAmount) as sum_sale from Sales
group by SaleDate
...
select departmentName, count(Firstname) as cnt_dpt from Employees
group by DepartmentName
having count(Firstname)>5

--7) Write a query to count the number of employees in each department.

select count(*) as cnt_emp from Employees 
--8) Write a query to show the minimum and maximum Price of products grouped by Category.
--Use products table.
...
--DepID is enough, if you don't have DeptName).

--14) Write a query to calculate the average salary per Department.
--eptID is enough, if you don't have DeptName).

select departmentName, avg(salary)  as avg_sal from Employees
group by departmentName

--15) Write a query to show the AVG salary and COUNT(*) of employees working in each department.
--ptID is enough, if you don't have DeptName).

select departmentName, avg(salary) as avg_sal, count(*) as cnt_emp from Employees
group by DepartmentName
