...
Select e.name as Employ_Name, e.salary as emp_salary, d.DepartmentName as DepartmentName from Employees e
Join Departments d
on e.DepartmentID=d.DepartmentID
where e.salary>50000

--2) Using the Customers and Orders tables, write a query to display customer names and order dates 
--for orders placed in the year 2023.
...
--3) Using the Employees and Departments tables, write a query to show all employees along with their 
--department names. Include employees who do not belong to any department.

select e.Name as Name, d.DepartmentName from Employees e
...
select e1.Name as EmployName, isnull(e2.Name, 'Boss') as Manager from Employees e1
...
--8) Using the Customers and Orders tables, write a query to find customers who have placed an order 
--with more than 3 items. Return their name and the quantity they ordered.

select c.FirstName, o.Quantity from customers c
Join Orders o
on c.CustomerID=o.CustomerID
where o.Quantity>3

--9) Using the Employees and Departments tables, write a query to list employees working in the 
...
--10) Using the Employees and Departments tables, write a query to return department names that have
--more than 5 employees.

select d.DepartmentName, count(e.name) as count from Employees e
Join Departments d
on e.DepartmentID=d.DepartmentID
...
--12) Using the Customers and Orders tables, write a query to return customer names who have placed at least
--one order.
...
select e1.name as Employee1, e2.Name as Employee2, e1.ManagerID as ManagerID from employees e1
join employees e2
on e1.ManagerID=e2.ManagerID
where e1.EmployeeID<e2.EmployeeID

--15) Using the Orders and Customers tables, write a query to list all orders placed in 2022 along with the
--customer name.
...
--19) Using the Employees table, write a query to find employees whose salary is greater than the average
--salary in their own departments.


select e.EmployeeID, e.Name, e.DepartmentID, e.Salary from Employees e
where e.Salary>(select avg(Salary) from Employees where  DepartmentID=e.DepartmentID);

--20) Using the Orders and Payments tables, write a query to list all orders placed before 2020 that
--have no corresponding payment.
...
--21)Using the Products and Categories tables, write a query to return products that do not
--have a matching category.

select * from Products p
full join categories c
on p.Category=CategoryName

--22) Using the Employees table, write a query to find employees who report to the same manager and
--earn more than 60000.

select e1.EmployeeID, e1.Name, e1.ManagerID,e1.Salary from Employees e1
join Employees e2
...
select products.ProductName, sales.saleid, sales.SaleAmount from Products
join Sales
