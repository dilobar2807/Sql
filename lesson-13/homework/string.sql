...
--1) You need to write a query that outputs "100-Steven King", meaning emp_id + first_name + last_name in 
--that format using employees table.

SELECT CONCAT(RTRIM(EMPLOYEE_ID), '-', RTRIM(First_NAME),' ', RTRIM(LAST_NAME)) AS Full_info 
FROM Employees

--2)Update the portion of the phone_number in the employees table, within the phone number the 
--substring '124' will be replaced by '999'

SELECT  Employee_id, First_name, Last_name, Phone_number, 
	CASE WHEN SUBSTRING(Phone_number, 5, 3)= '124' THEN 
	REPLACE (Phone_number, substring(Phone_number, 5, 3), '999') 
	ELSE  Phone_number   END AS Nem_number
FROM Employees

--3) That displays the first name and the length of the first name for all employees whose name starts 
--with the letters 'A', 'J' or 'M'. Give each column an appropriate label. Sort the results by the employees' 
--first names.(Employees)

SELECT first_name AS NAME, LEN(First_Name) AS Len_of_Name FROM Employees
...
SELECT FIRST_NAME, MANAGER_ID, SUM(Salary)  AS TOTAL_SALARY FROM EMPLOYEES
GROUP BY  First_Name, MANAGER_ID

--5)Write a query to retrieve the year and the highest value from the columns Max1, Max2, and Max3 for each row 
--in the TestMax table

SELECT *,
	CASE WHEN  Max1>Max2 and Max1>Max3 THEN Max1
		WHEN Max2>Max1 and Max2>Max3 THEN Max2
	ELSE Max3 END AS Max_value 
FROM TestMax
...
CHarindex(' ', Fullname)-1) as Middname,
substring(Fullname, charindex(' ', Fullname, charindex(' ', fullname)+1), len(fullname)) as LastName  

from Students

--2) For every customer that had a delivery to California, provide a result set of the customer orders 
--that were delivered to Texas. (Orders Table)
...
where
len(concat(First_name, Last_name))-len(replace(concat(First_name, Last_name), 'a', ''))>=3 

--5) The total number of employees in each department and the percentage of those employees who have
--been with the company for more than 3 years(Employees)
...
cast(count(EMPLOYEE_ID) as decimal(10, 2)) / (select count(*) from Employees) * 100 as percentage
from Employees
where DATEDIFF(year, hire_date, GETDATE()) > 3
group by department_id

--6)Write an SQL statement that determines the most and least experienced Spaceman ID by their job 
--description.(Personal)
-- Eng ko‘p tajribaga ega bo‘lganlar
SELECT JobDescription, SpacemanID, MissionCount AS Most_Experienced
FROM Personal
WHERE (Job_Description, MissionCount) IN (
    SELECT JobDescription, MAX(MissionCount)
    FROM Personal
    GROUP BY JobDescription
);

-- Eng kam tajribaga ega bo‘lganlar
SELECT JobDescription, SpacemanID, MissionCount AS Least_Experienced
FROM Personal
WHERE (JobDescription, MissionCount) IN (
    SELECT JobDescription, MIN(MissionCount)
    FROM Personal
    GROUP BY JobDescription
);
...
DECLARE @text NVARCHAR(100) = 'tf56sd#%OqH';

SELECT
  @text AS Original_Text,
  (SELECT STRING_AGG(value, '') 
   FROM STRING_SPLIT(@text, '') 
   WHERE value COLLATE Latin1_General_BIN LIKE '[A-Z]') AS Uppercase_Letters,
  (SELECT STRING_AGG(value, '') 
   FROM STRING_SPLIT(@text, '') 
   WHERE value COLLATE Latin1_General_BIN LIKE '[a-z]') AS Lowercase_Letters,
  (SELECT STRING_AGG(value, '') 
   FROM STRING_SPLIT(@text, '') 
   WHERE value LIKE '[0-9]') AS Numbers,
  (SELECT STRING_AGG(value, '') 
   FROM STRING_SPLIT(@text, '') 
   WHERE value COLLATE Latin1_General_BIN NOT LIKE '[A-Za-z0-9]') AS Other_Characters;
