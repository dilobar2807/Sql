...
select c.FirstName, o.OrderID, o.OrderDate from Orders o
Join  Customers c
...
on e.DepartmentID=d.DepartmentID and DepartmentName='Sales' or d.DepartmentName='Marketing'

--3)Task: Show the highest salary for each department.
--Return: DepartmentName, MaxSalary
--Tables Used: Departments, Employees

select d.DepartmentName, max(salary)  as Max_Salary from Departments d
...
select c.FirstName, c.LastName, c.Country  from Customers c
left join  Orders o
on c.CustomerID=o.CustomerID and c.Country='USA' and year(o.OrderDate)=2023

--5) Task: Show how many orders each customer has placed.
--Return: CustomerName, TotalOrders
--Tables Used: Orders , Customers

select c.CustomerID, c.FirstName, count(o.orderid) as Total_order from Customers c
Join Orders o
...
select  c.customerid,  c.FirstName, max(o.OrderDate) as MostRecentOrder from Customers c
join Orders o
on c.CustomerID=o.CustomerID 
group by c.CustomerID, c.FirstName
...
select s.Productid, p.productname,  sum(s.SaleAmount) as TotalAmount  from Sales s
...
select c.FirstName, o.OrderID, c.Address, o.OrderDate, c.Country from Customers c
join orders o
on c.CustomerID=o.CustomerID and c.Address like '[0-9][0-9][0-9][0-9]%' and c.Country='USA'
...
select c.CategoryName, c.CategoryID, sum(p.stockquantity) as ProductCount from Categories  c
left Join Products p
...
select c.FirstName, c.City, o.TotalAmount from Customers c
left join Orders o
on c.customerid=o.customerid and c.city='Los Aneles' and o.TotalAmount>300

--18) Task: Display employees who are in the HR or Finance department, or whose name contains at least 
--4 vowels.
--Return: EmployeeName, DepartmentName
--Tables Used: Employees, Departments
...
select e.Name, d.DepartmentName from Employees e
join Departments d 
on e.DepartmentID=d.DepartmentID and d.DepartmentName in ('Human Recoueses', 'Finance') 
or  (e.Name like '%a%' and e.name like '%o%' and e.name like '%e%' and e.name like '%i%')
