...
Select c.FirstName, o.orderID from Customers c
Join Orders o
...
select name, d.departmentname from Employees e
JOIN Departments d
...
--11) Using Employees, Departments table List employee names and department names where 
...
select o.OrderID, o.ProductID, p.ProductName, o.Quantity as Ordered_Quantity from Orders o
 JOIN Products p
on o.ProductID=p.ProductID
...
select c.FirstName, c.Country from Customers c
JOIN Sales s
on c.CustomerID=s.CustomerID 
where Country='USA'
...
select o.OrderID, o.TotalAmount, c.Country from Orders o
JOIN Customers c
