...
select ProductID, sum(saleAmount) as sum_sale from Sales
group by ProductID
...
create table city_population ( district_id int,
...
select category, count(ProductName) as cnt_pro from Products
group by category

--2) Using Products table, get the average price of products in the 'Electronics' category.

select avg(price) as avg_prc from Products
...
select max(price) as max_prc from Products
...
select country, count(Firstname) as cnt_name from Customers
group by Country

--9) Using Orders table, find the minimum and maximum quantity ordered.
select min(Quantity) as min_quan, max(quantity) as max_quan from Orders
...
select  Orders.CustomerID, Orders.OrderDate from Orders
JOIN Invoices
...
select YEAR(OrderDate) as YEAR, avg(TotalAmount) as avg_amn from Orders
...
select Bektemir, Chilonzor, Yakkasaroy  into Population_Each_City
from 
(
...
select Products.ProductName, count(SaleID) as TimesSold  from Products
JOIN Sales
